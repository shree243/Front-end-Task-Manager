...
const BoardUser = () => {
  ...
  return (
    <>
      <button className="grey-button" onClick={downloadFile}>Download Todo's Report</button>
          ...
          dateImportant.map((item, index) => (
            <div style={{ marginTop: '3%' }}>
              <Alert severity="warning">
                ...
                Todo has Due in 2 Days<strong> -- check it out!</strong>
              </Alert>
            </div>
          ))
        )}
      ...
      <div style={{ position: 'fixed', top: '20px', right: '20px', zIndex: 9999 }}>
        {alert && (
          <Alert severity="success">
            ...
            Todo has Been Created<strong>--check it out!</strong>
          </Alert>
        )}
      </div>
      ...
      <Box sx={{ width: '100%' }}>
        ...
        >
          <Tab value={0} label="All Tasks" style={{ color: 'white' }} />
          <Tab value={1} label="Important Tasks" style={{ color: 'white' }} />
          <Tab value={2} label="Completed Tasks" style={{ color: 'white' }} />
          <Tab
            value={3}
            label={
      ...
      <div>
        {/* <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-around', padding: '20px' }}>
          {allUsersTodos.map((todo) => <RenderCard todo={todo} />)}
          {allUsersImportantTodos.map((todo) => <RenderCard todo={todo} />)}
          {allUsersCompletedTodos.map((todo) => <RenderCard todo={todo} />)}
        </div> */}
        {/* allUsersImportantTodos, setAllUsersImportantTodos] = useState([{}]) */}
        {/* const [allUsersCompletedTodos */}
      </div>
    </>
